            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm mb-2" style={{ color: '#666666' }}>
                  {'📅 '}
                  {new Date(ev.date_debut).toLocaleDateString('fr-CA', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                  {ev.date_fin !== ev.date_debut && (
                    <>{' → '}{new Date(ev.date_fin).toLocaleDateString('fr-CA', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</>
                  )}
                </p>

                {ev.lieu && (
                  <p className="text-sm mb-2" style={{ color: '#666666' }}>{'📍 '}{ev.lieu}</p>
                )}

                {ev.villes && ev.villes.length > 0 && (
                  <p className="text-sm mb-2" style={{ color: '#666666' }}>{'🏙️ '}{ev.villes.map(v => v.nom_ville).join(' · ')}</p>
                )}
              ...
              <div className="flex flex-col items-end gap-3 flex-shrink-0">
                {ev.a_billets ? (
                  <>
                    ...
                    <a href={urlAchat} className="px-5 py-2.5 rounded-lg text-white font-medium text-sm inline-block" style={{ backgroundColor: '#C9A84C' }}>Acheter un billet</a>
                  </>
                ) : (
                  <span className="text-sm" style={{ color: '#666666' }}>Événement informatif</span>
                )}
              </div>
