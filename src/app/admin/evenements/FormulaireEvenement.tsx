    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Description */}
          <div>
            <label className={labelClass} style={{ color: '#1A2535' }}>Description (bouton "Plus d'informations")</label>
              ...
              {villes.map((ville, i) => (
                <div key={i} className="flex gap-2">
                  <input
                    ...
                    placeholder={`Ville ${i + 1}`}
                    value={ville.nom_ville}
                    onChange={e => updateVille(i, e.target.value)}
                  />
                  {villes.length > 1 && (
          ...
          {form.a_billets && (
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className={labelClass} style={{ color: '#1A2535' }}>
                  Paliers de prix (max. 4)
                </label>
                {paliers.length < 4 && (
              ...
              <div className="space-y-2">
                {paliers.map((palier, i) => (
                  <div key={i} className="flex gap-2 items-center">
                    <div className="w-8 text-sm text-center font-medium" style={{ color: '#666666' }}>
                      #{i + 1}
                    </div>
                    <input
                      type="number"
                      ...
                      placeholder="Prix ($)"
                      value={palier.prix || ''}
                      onChange={e => updatePalier(i, 'prix', parseFloat(e.target.value))}
                    />
                    <input
                      type="datetime-local"
                      className={inputClass}
                      ...
                      value={palier.date_fin ? palier.date_fin.slice(0, 16) : ''}
                      onChange={e => updatePalier(i, 'date_fin', e.target.value)}
                    />
                    {paliers.length > 1 && (
              ...
              <p className="text-xs mt-1" style={{ color: '#666666' }}>
                Le prix change automatiquement à la date de fin de chaque palier.
              </p>
            </div>
          )}
        ...
        <div className="flex justify-between p-6 border-t" style={{ borderColor: '#E0E0E0' }}>
          <button onClick={onFermer} className="px-4 py-2 rounded-lg text-sm" style={{ color: '#666666', backgroundColor: '#F5F3EE' }}>
            Annuler
          </button>
          <button
