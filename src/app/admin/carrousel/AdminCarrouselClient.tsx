...
export default function AdminCarrouselClient() {
  ...
  return (
    <div className="p-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-1" style={{ color: '#1A2535' }}>Carrousel d'images</h1>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
          <h2 className="text-lg font-semibold mb-1" style={{ color: '#1A2535' }}>Ajouter une image</h2>
          <p className="text-sm text-gray-400 mb-4">Format recommandé : <strong>16:9</strong>, min 1200×675px, JPG, max 3MB</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-lg font-semibold" style={{ color: '#1A2535' }}>Images ({images.length})</h2>
                ...
                >
                  <div className="flex items-center gap-4 p-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {img.lien ? (
                        <p className="text-xs mt-1 truncate font-medium" style={{ color: '#2E86C1' }}>🔗 {img.lien}</p>
                      ) : (
                        <p className="text-xs mt-1 text-gray-300">Aucun lien</p>
                      )}
                    </div>
                    <div className="flex gap-2 flex-shrink-0">
                      <button onClick={() => setEditLien({ id: img.id, lien: img.lien ?? '' })} className="text-xs font-medium px-3 py-1 rounded-lg hover:bg-blue-50 transition-colors" style={{ color: '#2E86C1' }}>
                        🔗 Lien
                      </button>
                      <button onClick={() => supprimerImage(img.id, img.url)} className="text-xs font-medium px-3 py-1 rounded-lg hover:bg-red-50 transition-colors" style={{ color: '#E57373' }}>
                        Supprimer
                      </button>
                    </div>
                  </div>
                  ...
                  {editLien?.id === img.id && (
                    <div className="px-4 pb-4 pt-1 border-t border-gray-100">
                      ...
                      <div className="flex gap-2">
                        ...
                        />
                        <button onClick={() => sauvegarderLien(img.id, editLien.lien)} className="px-4 py-2 rounded-lg text-sm font-medium text-white" style={{ backgroundColor: '#4CAF7D' }}>
                          Sauvegarder
                        </button>
                        <button onClick={() => setEditLien(null)} className="px-4 py-2 rounded-lg text-sm font-medium" style={{ color: '#666666', backgroundColor: '#F5F3EE' }}>
                          Annuler
                        </button>
                      </div>
                    </div>
