...
export default function ScanInterface({ compte, evenement, ville, evenements }: Props) {
  ...
  const [villeChoisie, setVilleChoisie] = useState(ville?.nom_ville ?? '')
  ...
  async function scanner(tokenScan: string) {
    ...
    const { data: billet } = await supabase
      ...
      .eq('qr_code_token', tokenScan.trim())
      .single()

    if (!billet) {
      setResultat({ type: 'erreur', message: '❌ BILLET INVALIDE', couleur: '#E57373' })
      setChargement(false)
      return
    }

    if (billet.statut === 'rembourse') {
      setResultat({ type: 'annule', message: '❌ BILLET ANNULÉ', sousTitre: 'Ce billet a été remboursé.', couleur: '#E57373' })
      setChargement(false)
      return
    }
    ...
    if (billet.statut === 'utilise') {
      ...
      setResultat({
        ...
        sousTitre: billet.nom_pci + ' · ' + dateScan,
        couleur: '#FF9800'
      })
      setChargement(false)
      return
    ...
    if (evenementActif && billet.evenement_id !== evenementActif.id) {
      setResultat({ type: 'erreur', message: '❌ BILLET INVALIDE', sousTitre: 'Mauvais événement.', couleur: '#E57373' })
      setChargement(false)
      return
    }

    await supabase.from('billets').update({
      statut: 'utilise',
      scanne_le: new Date().toISOString(),
      scanne_ville: villeChoisie || null,
    }).eq('id', billet.id)

    const abanquet = billet.evenement?.a_banquet
    let message = ''
    const sousTitre = billet.nom_pci
    ...
    setResultat({ type: 'succes', message, sousTitre, couleur: '#4CAF7D' })
    setToken('')
    setChargement(false)
  }
  ...
  async function rechercherManuel() {
    ...
    if (trouves.length === 0) {
      ...
      // Toujours afficher les résultats pour validation
    ...
    <div className="space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow p-4 space-y-3">
          <div>
            ...
            <select
              ...
              value={evenementChoisi}
              onChange={e => setEvenementChoisi(e.target.value)}
            >
          ...
          {evenementActif?.villes?.length > 0 && (
            <div>
              ...
              >
                ...
                {evenementActif.villes.map((v: any) => (
      ...
      {resultat && (
        <div className="rounded-2xl p-6 text-center" style={{ backgroundColor: resultat.couleur }}>
          ...
          {resultat.sousTitre && (
            <p className="text-white text-sm mt-1 opacity-90">{resultat.sousTitre}</p>
          )}
        </div>
      )}

      {/* Résultats de recherche — validation */}
          ...
          <div className="space-y-2">
            {billetsRecherche.map(b => (
              <div key={b.id} className="flex items-center justify-between p-3 rounded-lg border"
                ...
                <button
                  onClick={() => confirmerScan(b)}
                  disabled={chargement}
                  className="px-4 py-2 rounded-lg text-white text-sm font-medium disabled:opacity-50"
                  style={{ backgroundColor: b.statut === 'utilise' ? '#FF9800' : '#4CAF7D' }}
                >
                  Confirmer
                </button>
              </div>
      ...
      <div className="bg-white rounded-2xl shadow p-4">
        ...
        <div className="flex gap-2">
          <input
            ...
            style={{ borderColor: '#E0E0E0' }}
            placeholder="Token du billet..."
            value={token}
            onChange={e => setToken(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && scanner(token)}
            autoFocus
          />
          <button
            onClick={() => scanner(token)}
            disabled={chargement || !token.trim()}
            className="px-4 py-2.5 rounded-lg text-white font-medium text-sm disabled:opacity-50"
        ...
        <p className="text-xs mt-2" style={{ color: '#999999' }}>
          Pointez un lecteur QR ou entrez le token manuellement.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow p-4">
        <label className="block text-sm font-medium mb-2" style={{ color: '#1A2535' }}>
          Recherche manuelle (nom)
        </label>
        <div className="flex gap-2">
          <input
            className="flex-1 border rounded-lg px-4 py-2.5 text-sm outline-none"
            style={{ borderColor: '#E0E0E0' }}
            placeholder="Prénom ou nom du PCI..."
            value={recherche}
            onChange={e => setRecherche(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && rechercherManuel()}
          />
          <button
            onClick={rechercherManuel}
