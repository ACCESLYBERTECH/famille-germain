...
export default function ContactPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (user) {
        supabase.from('comptes').select('prenom_1, role').eq('id', user.id).single().then(({ data }) => {
          if (data) {
            setPrenom(data.prenom_1 ?? '')
            setRole(data.role ?? '')
          }
        })
      }
  ...
  if (envoye) {
    return (
      <div className="min-h-screen" style={{ backgroundColor: '#F5F3EE' }}>
        <Navbar prenom={prenom} role={role} />
        <div className="max-w-xl mx-auto pt-12 px-4 text-center">
          <div className="bg-white rounded-2xl shadow-lg p-8">
            ...
            <p className="text-sm mb-6" style={{ color: '#666666' }}>Nous vous répondrons dans les plus brefs délais.</p>
